...
def main():
    ...
    #     device = torch.device("cuda" if tf.cuda.is_available() and not args.no_cuda else "cpu")
    ...
    config_class, model_class, tokenizer_class = MODEL_CLASSES[args.model_type]     #加载预训练模型
    config = config_class.from_pretrained(args.config_name if args.config_name else args.model_name_or_path)
    tokenizer = tokenizer_class.from_pretrained(args.tokenizer_name if args.tokenizer_name else args.model_name_or_path,do_lower_case=args.do_lower_case)
    
    #budild model
    encoder = model_class.from_pretrained(args.model_name_or_path,config=config)  #加载roberta作为encoder
    ...
    model=Seq2Seq(encoder=encoder,decoder=decoder,config=config,
                  ...
                  sos_id=tokenizer.cls_token_id,eos_id=tokenizer.sep_token_id)  #调用了model.py 中的Seq2Seq
    # if args.load_model_path is not None:
    #     logger.info("reload model from {}".format(args.load_model_path))
    #     model.load_state_dict(torch.load(args.load_model_path))  #加载预训练模型的参数
    ...
    if args.do_train:
        ...
        train_features = convert_examples_to_features(train_examples, tokenizer,args,stage='train')  # 将word按照词典匹配成feature，比较耗费资源
        all_source_ids = tf.concat(0,[f.source_ids for f in train_features])  #将所有的source_ids整合在一个tensor中，下面的也是一样
        all_source_mask = tf.concat(0,[f.source_mask for f in train_features])
        all_target_ids = tf.concat(0,[f.target_ids for f in train_features])
        all_target_mask = tf.concat(0,[f.target_mask for f in train_features])
        train_data = TensorDataset(all_source_ids,all_source_mask,all_target_ids,all_target_mask)   #准备好训练数据
        ...
        scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=args.warmup_steps,
                                                    num_training_steps=num_train_optimization_steps)  #学习率预热
